...
class RoutingExperiment
{
  ...
  private:
    ...
    std::string m_CSVfileName{"blackhole-aodvx3.csv"}; //!< CSV filename.
...
protected:
    virtual void RecvAodv (Ptr<Packet> packet, const Ipv4Header & header, Ptr<const NetDevice> receiver, const Address & source);

private:
    void HandleBlackHole(Ptr<Packet> packet, const Ipv4Header & header, Ptr<const NetDevice> receiver, const Address & source);
};
...
void
BlackHoleAodv::DoDispose()
{
    aodv::RoutingProtocol::DoDispose();
}

void
BlackHoleAodv::RecvAodv(Ptr<Packet> packet, const Ipv4Header & header, Ptr<const NetDevice> receiver, const Address & source)
{
    // Implement black hole behavior
    HandleBlackHole(packet, header, receiver, source);
}

void
BlackHoleAodv::HandleBlackHole(Ptr<Packet> packet, const Ipv4Header & header, Ptr<const NetDevice> receiver, const Address & source)
{
...
RoutingExperiment::Run()
{
    ...
    // Set up WiFi, mobility, and other configurations
    WifiHelper wifi;
    ...
    pos.Set("Y", StringValue("ns3::UniformRandomVariable[Min=0.0|Max=4500.0]"));
    ...
    for (int i = 0; i < m_nSinks; i++)
    {
        ...
        temp.Start(Seconds(var->GetValue(300.0, 303.0)));
    ...
    AnimationInterface anim ("blackhole-aodvx3.xml");
